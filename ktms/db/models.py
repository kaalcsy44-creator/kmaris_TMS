...
class Vendor(Base):
    ...
    # 이 거래선이 곧 그 제조사다 — 벤더를 거치지 않고 메이커에 직접 물어 사는 경우
    # (2단계 RFQ Sent 의 수신처를 Maker 로 골랐을 때 명부에서 옮겨 심은 줄).
    # maker_ids 와 뜻이 다르다: 저것은 '누구 것을 대 주나'이고 이것은 '이 회사가 곧 그
    # 제조사'다. 견적 수신·P/O·지급은 언제나 거래선을 타고 흐르므로, 메이커 직거래도
    # 여기 한 줄로 서야 그 뒤가 이어진다. NULL = 보통의 거래선.
    maker_id       = Column(Integer, ForeignKey("makers.id"), nullable=True)
    website        = Column(String(300))   # 회사 홈페이지(회사 단위)
...
class Maker(Base):
    ...
    담당자는 거래선과 같은 규약으로 둔다 — 레코드 1건 = 담당자 1명. 메이커에도 물어볼
    사람이 있다(대리점이 없는 브랜드의 기술문의, 단종품 확인, 정품 여부). 그 창구가
    영업과 기술로 갈리는 것도 거래선과 같아서, 회사 밑에 사람을 여럿 달 수 있어야 한다.
    회사 단위 값(주소·홈페이지·분류·소개·로고)은 같은 이름의 레코드 전부에 한꺼번에
    반영한다(Company info) — 거래선·고객과 같은 장치를 그대로 쓴다.

    **makers.id 는 회사가 아니라 그 레코드(담당자 한 줄)를 가리킨다.** 거래선의
    '대 줄 수 있는 제조사'(Vendor.maker_ids)와 메이커-거래선 링크(Vendor.maker_id)가
    이 id 를 들고 있으므로, 담당자 줄을 지울 때는 같은 회사의 남은 줄로 그 참조를 옮겨
    준다(routers/settings.delete_maker). 고르는 화면은 회사 이름으로 중복을 걷어내
    한 회사를 한 번만 보여 준다(MakerCell.useMakerCompanies).

    메이커에 직접 물어보는 일은 있다(대리점이 없는 브랜드, 단종품 문의). 그때는 그
    메이커가 이 명부에 남은 채로 거래선 명부에도 한 줄 선다(Vendor.maker_id 가 그
    둘을 잇는다) — 견적 수신·P/O·지급이 모두 거래선을 타고 흐르기 때문이다. 이 표는
    여전히 '누가 만들었나'만 답하고, '어디서 사는가'는 거래선이 답한다.
    """
    ...
    country        = Column(String(100))   # 대표 지역(regions[0] 미러링)
    address        = Column(String(400))   # 대표 주소(addresses[0] 미러링)
    # 담당자 — 거래선과 같은 칸(레코드 1건 = 담당자 1명). 비어 있으면 회사 대표 창구다
    # (기술문의 주소·본사 대표번호). 그래서 담당자가 없는 메이커도 줄 하나로 선다.
    contact        = Column(String(100))   # 담당자 이름
    duty           = Column(String(100))   # 담당 분야(영업·기술·A/S)
...
class ARRecord(Base):
    ...
    # 본 계약 청구(main)인가 추가비용 청구(extra)인가.
    #
    # 한 오더에 청구서가 여럿 설 수 있다 — 일정이 밀려 항공권을 바꾸거나 비자를 새로 받는
    # 식으로 본 계약과 별개인 비용이 생기면, 그 몫을 따로 끊는다(9단계 AR 탭의 청구서
    # 스트립). 견적은 3·4단계에 이미 한 건 더 서 있고, 이것은 그 견적의 청구서다.
    #
    # 표시를 남기는 까닭은 단계 판정 때문이다. '이 딜의 청구서'를 찾는 자리는 거의 다 본
    # 계약 건을 뜻하는데, _deal_progress 가 추가 청구를 함께 세면 두 군데가 무너진다 —
    # ar_billed 가 추가 청구 하나로 참이 되어 9단계가 앞서 초록이 되고, ar_paid 는 any()
    # 라 추가 건만 완납돼도 11단계가 완료로 보인다. 그래서 그쪽은 kind != "extra" 로 세고,
    # 추가 청구의 미수는 딜 카드의 배지로 따로 알린다.
